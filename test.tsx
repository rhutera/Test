...
const { BREAK_POINTS, SELECTION_DATA_VIEW_ID, DATA_VIEW_ID_FOR_NO_SELECTION } =
	CONSTANTS;
...
export interface Props {
	...
	viewInTableObj: {
		[id: string]: { daLayerItem: LayersConfig; records: DataRecord[] };
	};
...
> {
	...
	static mapExtraStateProps = (
		...
		for (const key in updatedViewInTableObj) {
			// update label
			const newLabel =
				updatedViewInTableObj[key]?.daLayerItem?.dataActionDataSource
					?.dataSourceJson?.label;
			if (newLabel)
				updatedViewInTableObj[key].daLayerItem.name = newLabel;
			// update runtime dataSource info
			const dataActionDataSourceId =
				updatedViewInTableObj[key]?.daLayerItem?.dataActionDataSource
					?.id;
			const dataActionDataSourceInfo =
				updatedViewInTableObj[
					key
				]?.daLayerItem?.dataActionDataSource?.getInfo();
			runtimeDataSourceInfos[
				`${runtimeDataSourceKey}_${dataActionDataSourceId}`
			] = dataActionDataSourceInfo;
		}
		return {
			...
			enableDataAction:
				enableDataAction === undefined ? true : enableDataAction,
			...runtimeDataSourceInfos,
			isHeightAuto,
			isWidthAuto
		};
	...
	constructor(props) {
		...
		this.lastSql = "";
		this.debounceOnResize = lodash.debounce((width, height) => {
	...
	static getDerivedStateFromProps(nextProps, prevState) {
		...
		const allLayersConfig = layersConfig
			.asMutable({ deep: true })
			.concat(daLayersConfig);
		if (
			(!activeTabId ||
				allLayersConfig.findIndex((x) => x.id === activeTabId) < 0) &&
			allLayersConfig.length > 0
		) {
			return {
	...
	componentDidUpdate(
		...
		const { id, config, currentPageId, state, appMode, viewInTableObj } =
			this.props;
		const { layersConfig } = config;
		...
		const newActiveTabId = dataActionActiveObj?.dataActionTable
			? dataActionActiveObj?.activeTabId
			: activeTabId;
		...
		const settingChangeTab =
			this.props?.stateProps?.settingChangeTab || false;
		const activeSettingTabId = this.props?.stateProps?.activeTabId;
		if (
			settingChangeTab &&
			activeSettingTabId &&
			activeTabId !== activeSettingTabId
		) {
			this.props.dispatch(
		...
		const liveClose =
			prevProps.appMode === AppMode.Run && appMode === AppMode.Design;
		...
		// config change
		const prevCurConfig = prevProps.config.layersConfig
			?.asMutable({ deep: true })
			.concat(daLayersConfig)
			.find((item) => item.id === prevState.activeTabId);
		...
		if (this.removeConfig) {
			...
			const viewInTableIds =
				viewInTableObj && Object.keys(viewInTableObj);
			if (
				(viewInTableIds?.length === 0 && !prevCurConfig) ||
				!newCurConfig
			)
				return;
		}
		// search close
		const orgSearchOn =
			prevCurConfig?.enableSearch && prevCurConfig?.searchFields;
		const newSearchOn =
			newCurConfig?.enableSearch && newCurConfig?.searchFields;
		if (orgSearchOn && !newSearchOn && dsAvailable) {
			dataSource.updateQueryParams(EMPTY_QUERY_PARAMS, id);
		}
		// setting options change
		const optionChangeFlag = this.settingOptionsChange(
			prevCurConfig,
			newCurConfig
		);
		const tabChange =
			!dataActionActiveObj?.dataActionTable &&
			prevCurConfig?.id !== newCurConfig?.id;
		const tableOptionChange =
			prevCurConfig?.id === newCurConfig?.id && optionChangeFlag;

		if (prevState.dataSource !== this.state.dataSource) {
			const curLayer = this.props.config.layersConfig.find(
				(item) => item.id === newActiveTabId
			);
			if (curLayer?.useDataSource) {
				this.setState({ tableTotal: 0 });
		...
		if (tabChange || tableOptionChange) {
			console.log("tabChange or tableOptionChange");
		...
		if (
			dataActionActiveObj?.dataActionTable &&
			this.dataActionCanLoad &&
			!this.updatingTable
		) {
			this.dataActionCanLoad = false;
			this.props.dispatch(
				appActions.widgetStatePropChange(id, "dataActionActiveObj", {
					activeTabId: newActiveTabId,
					dataActionTable: false
				})
			);
			this.updatingTable = true;
			this.setState(
	...
	getTimezone = (dataSource) => {
		return dataSourceUtils.getTimezoneAPIFromRuntime(
	...
	onSettingChangeRuntime = () => {
		const { id } = this.props;
		const optionChangeSuggestion =
			this.props?.stateProps?.optionChangeSuggestion || false;
		const removeLayerFlag =
			this.props?.stateProps?.removeLayerFlag || false;
		if (optionChangeSuggestion) {
			this.setState({ isShowSuggestion: false });
			this.props.dispatch(
				appActions.widgetStatePropChange(
					id,
					"optionChangeSuggestion",
					false
				)
			);
		}
	...
	onFilterChange = (newActiveTabId: string) => {
		...
		if (viewInTableObj && activeTabId === newActiveTabId) {
			...
			const currentTabViewTableIndex = viewInTableIds.findIndex(
				(a) => a === newActiveTabId
			);
			if (currentTabViewTableIndex > -1) {
				const activeViewDs =
					viewInTableObj[newActiveTabId].daLayerItem
						.dataActionDataSource;
				const activeUseDs =
					viewInTableObj[newActiveTabId].daLayerItem.useDataSource;
				let dsParam: any;
				if (activeViewDs) {
					dsParam = (
						activeViewDs as QueriableDataSource
				...
				if (
					orgExpression !== undefined &&
					orgExpression !== dsParam?.where
				) {
	...
	// setting options change
	settingOptionsChange = (
		prevCurConfig: LayersConfig,
		newCurConfig: LayersConfig
	): boolean => {
		...
		for (const item of optionKeys) {
			const changeFlag = referenceTypeKeys.includes(item)
				? !lodash.isDeepEqual(
						prevCurConfig?.[item],
						newCurConfig?.[item]
				  )
				: prevCurConfig?.[item] !== newCurConfig?.[item];
			if (changeFlag) {
	...
	getFieldsFromDatasource = () => {
		...
		const allLayersConfig = layersConfig
			.asMutable({ deep: true })
			.concat(daLayersConfig);
		const curLayer = allLayersConfig.find(
			(item) => item.id === activeTabId
		);
		// 'allFields' need recalculate(chart output ds), if dataActionDataSource exists, use it
		const selectedDs =
			curLayer?.dataActionDataSource ||
			this.dsManager.getDataSource(curLayer.useDataSource?.dataSourceId);
		const allFieldsSchema = selectedDs?.getSchema();
		const allFields = allFieldsSchema?.fields
			? Object.values(allFieldsSchema?.fields)
			: [];
	...
	onDataSourceCreated = (dataSource: QueriableDataSource): void => {
		...
		const isSelectionView =
			dataSource?.dataViewId === SELECTION_DATA_VIEW_ID;
		...
		const isDataView =
			...
			dataSource?.dataViewId !== "output" &&
			!isSelectionView;
		const hasNoSelectionView = !!dataSource
			.getMainDataSource()
			.getDataView(DATA_VIEW_ID_FOR_NO_SELECTION);
		const dsChangeCreateTable =
			!this.updatingTable &&
			((isSelectionView && hasNoSelectionView) || isDataView);
	...
	updateGeometryAndSql = (dataSource: QueriableDataSource) => {
		...
		const isDefaultExpression =
			orgExpression === "" && dsParam?.where === "1=1";
		...
		dataSourceUtils
			.changeJimuFeatureLayerQueryToJSAPILayerQuery(
			...
			.then((res) => {
				...
				const { isWGS84, isWebMercator } =
					sr as __esri.SpatialReference;
				const useBufferMethod =
					isWGS84 || isWebMercator
						? this.geometryEngine.geodesicBuffer
						: this.geometryEngine.buffer;
				const orgGeometryJson = (
					this.table?.filterGeometry as any
				)?.toJSON();
				if (geometryType !== "polygon" && distance && distance <= 0) {
					...
					const geometryBuff = useBufferMethod(
						geometry as any,
						distance,
						units as any
					);
					if (
						!lodash.isDeepEqual(
							orgGeometryJson,
							(geometryBuff as __esri.Polygon)?.toJSON()
						)
					) {
	...
	onDataSourceInfoChange = (
		...
		if (
			info?.status === DataSourceStatus.Loaded &&
			preInfo?.status === DataSourceStatus.Loaded
		) {
			this.dataSourceChange = false;
		}
		...
		const isViewSnapshot =
			curLayer.dataActionType === TableDataActionType.View;
		// If other widgets load data, status will be loaded at the first time
		// This time state.dataSource is undefined
		if (
			(!dataSource && useDS) ||
			(!isViewSnapshot && dataSource?.id !== useDS?.dataSourceId)
		) {
			dataSource = this.dsManager.getDataSource(
				useDS?.dataSourceId
		...
		const isEmptySelectionView =
			dataSource?.dataViewId === SELECTION_DATA_VIEW_ID &&
			dataSource?.getSourceRecords()?.length === 0;
		if (!curLayer.dataActionObject) {
			if (
				!info?.status ||
				info?.status === DataSourceStatus.NotReady ||
				isEmptySelectionView
			) {
				this.destoryTable().then(() => {
		...
		const infoStatusNotChange =
			...
			(preSourceVersion === newSourceVersion ||
				newSourceVersion === this.tableSourceVersion + 1) &&
		...
		const tabChange =
			curLayer?.useDataSource?.dataSourceId !== dataSource?.id;
		const outputReapply =
			(!preInfo?.status || notLoad.includes(preInfo?.status)) &&
			info &&
			!notLoad.includes(info?.status) &&
			info?.instanceStatus !== DataSourceStatus.NotCreated;
		const selectedChange =
			preSelectedIds !== newSelectedIds &&
			(preSelectedIds?.length !== 0 || newSelectedIds?.length !== 0);
		const infoNotChange =
			info?.status === preInfo?.status &&
			info?.instanceStatus === preInfo?.instanceStatus;
		const isOutputDs = dataSource?.getDataSourceJson()?.isOutputFromWidget;
		const dsCreated =
			!curLayer.dataActionObject &&
			info?.status === DataSourceStatus.Unloaded &&
			info?.instanceStatus === DataSourceStatus.Created &&
			!selectedChange &&
			!infoNotChange &&
			!isOutputDs;
		if (
		...
		if (preSelectedIds !== newSelectedIds) {
			if (selectQueryFlag) {
				this.asyncSelectedWhenSelection(
					newSelectedIds || Immutable([])
				);
				setTimeout(() => {
					this.asyncSelectedRebuild(dataSource);
				}, 500);
	...
	compareAndUpdateTable = (
		...
		const {
			widgetQueries: preWidgetQueries,
			gdbVersion: preGdbVersion,
			sourceVersion: presSourceVersion
		} = preDsQueryInfo;
		const { widgetQueries, gdbVersion, needRefresh, sourceVersion } =
			curDsQueryInfo;
		const gdbVersionChange =
			preGdbVersion &&
			gdbVersion &&
			gdbVersion !== preGdbVersion &&
			this.table;
		const widgetQueryChange =
			!curLayer.dataActionObject && widgetQueries !== preWidgetQueries;
		const belongtoSourceVersionChange =
			isBelongto && presSourceVersion !== sourceVersion;
	...
	onQueryRequired = (
		queryRequiredInfo: QueryRequiredInfo,
		preQueryRequiredInfo?: QueryRequiredInfo
	) => {
		const { dataSource } = this.state;
		const isDataActionTable =
			this.props?.stateProps?.dataActionActiveObj?.dataActionTable;
		...
		if (time) {
			const apiTime =
				dataSourceUtils.changeJimuTimeToJSAPITimeExtent(time);
			const orgTimeExtent = tableLayer?.timeExtent;
			const timeNotChange =
				time?.[0] === orgTimeExtent?.start?.getTime() &&
				time?.[1] === orgTimeExtent?.end?.getTime();
	...
	getFeatureLayer = (
		...
		const notToLoad =
			dataSource?.getDataSourceJson()?.isDataInDataSourceInstance;
		let featureLayer;
		if (dataRecords?.length > 0) {
			return dataSourceUtils.createFeatureLayerByRecords(
				ds,
				dataRecords as FeatureDataRecord[]
			);
		} else {
			const curQuery: any =
				dataSource && dataSource.getCurrentQueryParams();
			if (notToLoad) {
				...
				const usingNoSelectionView =
					ds.dataViewId === SELECTION_DATA_VIEW_ID &&
					(!(ds as any).sourceRecords ||
						(ds as any).sourceRecords.length === 0);
				// If widget is using no_selection view, should use pageSize of the no_selection view to load records.
				if (usingNoSelectionView) {
					const noSelectionView = ds
						.getMainDataSource()
						.getDataView(DATA_VIEW_ID_FOR_NO_SELECTION);
					if (noSelectionView) {
						q.pageSize = ds
							.getMainDataSource()
							.getDataSourceJson()?.dataViews?.[
							DATA_VIEW_ID_FOR_NO_SELECTION
						]?.pageSize;
					}
				}
				// chart output and selected features need load
				return ds.query(q).then(async ({ records }) => {
					const dataRecords = (await Promise.resolve(
						records
		...
		if (notToLoad) {
			// output ds (dynamic layer, load will rise bug)
			return Promise.resolve(featureLayer);
		} else {
			// need load to get layer.capabilities
	...
	getFieldEditable = (layerDefinition, jimuName: string) => {
		...
		const orgField = fieldsConfig.find(
			(config) => config.name === jimuName
		);
	...
	dsAsyncSelectTable = (
		dataSource,
		selectedItems: DataRecord[] | number[],
		rowClick: boolean,
		versionChangeClear?: boolean
	) => {
		const { id } = this.props;
	...
	isDataSourceAccessible = (
		...
		if (actionDataSource) {
			...
			dataSourceIsInProps = isDataAction
				? hasInstance
				: this.props.useDataSources?.filter(
						(useDs) => dataSourceId === useDs.dataSourceId
				  ).length > 0;
		}
	...
	getIsAdvancedPermission = async (currentLayerDs): Promise<boolean> => {
		...
		const layerItemInfo = await currentLayerDs
			?.fetchItemInfo()
			.then((info) => {
				return info;
			})
		...
		const portalUrl =
			ServiceManager.getInstance().getServerInfoByServiceUrl(
				layerItemInfo.url
			)?.owningSystemUrl;
		if (!portalUrl) return false;
		const sessionForItem =
		...
		const isInUpdatedGroup = await privilegeUtils.isItemInTheUpdatedGroup(
			layerItemInfo.id,
			sessionForItem
		);
	...
	getLayerAndNewTable = (
		...
		const dataSourceUsed = isSceneLayer
			? (dataSource as SceneLayerDataSource).getAssociatedDataSource()
			: dataSource;
		if (dataSourceUsed) {
			const getPrivilege = () => {
				return privilegeUtils
					.checkExbAccess(privilegeUtils.CheckTarget.Experience)
					.then((exbAccess) => {
						return exbAccess?.capabilities?.canEditFeature;
					});
			};
			const tablePromise = cancelablePromise.cancelable(
				this.getFeatureLayer(dataSourceUsed, dataRecords).then(
					async (layer) => {
						if (
							newId !== this.currentRequestId ||
							!layer ||
							!this.FeatureTable ||
							!this.refs.currentEl ||
						...
						if (dataRecords?.length > 0) {
							...
							const newSourceLabel = `${
								dataSourceUsed.belongToDataSource?.getDataSourceJson()
									.sourceLabel
							}-${this.viewTableId}`;
							...
							layerDs = await this.dsManager.createDataSource(
								layerDsOption
							);
							this.setState({
								dataSource: layerDs as QueriableDataSource
							});
						}
						// fetch to confirm whether it's a public source
						const accessible = await this.getDsAccessibleInfo(
							layer?.url
						);
						...
						const isAdvancedPermission =
							await this.getIsAdvancedPermission(
								layerDs || dataSource
							);
						// full editing privileges
						const fullEditingPrivileges =
							featureLayer?.userHasFullEditingPrivileges;
						const layerEditingEnabled =
							featureLayer?.editingEnabled ?? true;
						const isHonorWebmap =
							curLayerConfig.layerHonorMode ===
							LayerHonorModeType.Webmap;
						const isNormalPermission = isHonorWebmap
							? accessible || privilegeEditable
							: curLayerConfig.enableEdit &&
							  (accessible || privilegeEditable);
						let editable = false;
						if (
							isAdvancedPermission ||
							(fullEditingPrivileges && layerEditingEnabled)
						) {
							editable = true;
						} else if (
							fullEditingPrivileges &&
							!layerEditingEnabled
						) {
							editable = false;
						} else {
						...
						const curColumns = tableShowColumns
							? tableShowColumns.map((col) => {
									return { jimuName: col.value };
							  })
							: curLayerConfig.tableFields.filter(
									(item) => item.visible
							  );
						...
						if (isHonorWebmap) {
							//  && dataSource.isDataView && dataSource?.dataViewId !== OUTPUT_DATA_VIEW_ID
							const popupInfo = (
								dataSource as FeatureLayerDataSource
							)?.getPopupInfo();
							const popupAllFieldInfos =
								popupInfo?.fieldInfos || [];
							// use schemaFields to filter used fields, some field is special and invisible in schema
							const schemaFieldsKeys = Object.keys(
								allFieldsSchema?.fields
							);
							const filteredPopupFieldInfos =
								popupAllFieldInfos.filter((item) =>
									schemaFieldsKeys.includes(item.fieldName)
								);
							const useFields =
								layerDefinition?.fields?.length > 0
									? layerDefinition.fields
									: allFields;
							const filteredUseFields = (
								useFields as any[]
							).filter((item) =>
								schemaFieldsKeys.includes(
									item.jimuName || item.name
								)
							);
							tableTemplate = popupInfo?.fieldInfos
								? new this.TableTemplate({
										columnTemplates:
											filteredPopupFieldInfos.map(
												(item) => {
													const itemKey =
														item.fieldName;
													return {
								...
								: new this.TableTemplate({
										columnTemplates: filteredUseFields.map(
											(item) => {
												const itemKey =
													item.jimuName || item.name;
												return {
							...
							tableTemplate = new this.TableTemplate({
								columnTemplates: curLayerConfig.tableFields.map(
									(item) => {
										const itemKey =
											item.jimuName || item.name;
										const newItem =
											allFieldsSchema?.fields?.[itemKey];
										return {
											fieldName: itemKey,
											label: newItem?.alias,
											...(editable
						...
						const deletable = this.getDsCapabilities(
							capabilities,
							"delete"
						);
						...
						if (
							tableTemplate?.columnTemplates?.length === 0 &&
							!isHonorWebmap
						) {
							this.resetUpdatingStatus();
							return;
						...
						if (editable) {
							...
							featureLayer.on("edits", function (event) {
								...
								const adds =
									addedFeatures && addedFeatures.length > 0;
								const updates =
									updatedFeatures &&
								...
								if (adds || updates || deletes) {
									const updateFeature =
										event?.edits?.updateFeatures?.[0];
									if (updateFeature) {
										const record =
											dataSource.buildRecord(
												updateFeature
											);
										const dsInfo = dataSource.getInfo();
										that.tableSourceVersion =
											dsInfo?.sourceVersion ?? 0;
						...
						if (timeExtent) {
							const apiTime =
								dataSourceUtils.changeJimuTimeToJSAPITimeExtent(
									timeExtent
								);
						...
						this.table = new this.FeatureTable({
							...
							attachmentsEnabled:
								curLayerConfig.enableAttachements,
							editingEnabled: editable,
							timeZone: this.getTimezone(dataSource)
						});
						// for table total/selected count
						reactiveUtils.watch(
							() => (this.table?.viewModel as any)?.size,
							(tableTotal) => {
								this.setState({ tableTotal });
							}
						);
						reactiveUtils.watch(
							() => this.table?.highlightIds?.length,
							(tableSelected) => {
						...
						reactiveUtils.watch(
							() =>
								this.table?.columns.some(
									(column) => !!(column as any).activeEditInfo
								),
							(isTableEditing) => {
						...
						const dataActionIsAdd =
							curLayerConfig.dataActionType ===
							TableDataActionType.Add;
						const dataActionObject =
							curLayerConfig.dataActionObject;
						if (!dataActionObject) {
							setTimeout(() => {
								if (selectQueryFlag)
									this.asyncSelectedWhenSelection(
						...
						if (curLayerConfig.enableSelect) {
							const rowClickFn = ({ context, native }) => {
								...
								this.timerFn = setTimeout(() => {
									...
									if (
										curLayerConfig.selectMode ===
										SelectionModeType.Single
									) {
										this.table.highlightIds.removeAll();
									}
									context.selected
										? this.table.highlightIds.remove(
												objectId
										  )
										: this.table.highlightIds.add(objectId);
									const selectedIds =
										tableInstance.grid?.highlightIds?.toArray();
						...
						tableInstance?.domNode?.addEventListener(
							"dblclick",
							(eve) => {
								clearTimeout(this.timerFn);
							}
	...
	createAllTables = () => {
		try {
			...
			allLayersConfig.forEach((layerConfig) => {
				if (layerConfig.id) {
					const curLayer = allLayersConfig.find(
						(item) => item.id === layerConfig.id
					);
					this.createTableByLyrConfig(layerConfig, curLayer);
				}
			});
		} catch (error) {
	...
	createTableByLyrConfig = (layerConfig, curLayer) => {
		let { dataSource } = this.state;
		const curLayerConfig = layerConfig;
		...
		const curDsId = dataActionDataSource
			? dataActionDataSource?.id
			: curLayerConfig?.useDataSource?.dataSourceId;
		const isCurDs =
			curLayerConfig.dataActionType === TableDataActionType.View ||
			curDsId === dataSource?.id;
		if (!isCurDs) {
			this.dataActionCanLoad = true;
			this.updatingTable = false;
			return;
	...
	createTable = (newDataSource?) => {
		...
		const allLayersConfig = layersConfig
			.asMutable({ deep: true })
			.concat(daLayersConfig);
		const curLayerConfig = allLayersConfig.find(
			(item) => item.id === activeTabId
		);
		if (!curLayerConfig) {
			this.dataActionCanLoad = true;
	...
	asyncSelectedRebuild = (dataSource: QueriableDataSource) => {
		...
		const belongToSelectedRecordIds =
			dataSource?.belongToDataSource?.getSelectedRecordIds() || [];
		if (
			dataSource.isDataView &&
			selectedRecordIds.length !== belongToSelectedRecordIds.length
		) {
		...
		if (
			curSelectRecords &&
			!lodash.isDeepEqual(selectRecords, curSelectRecords)
		) {
		...
		if (selectedRecordIds?.length > 0) {
			selectedRecordIds.forEach((recordId) => {
				if (recordId)
					this.table?.highlightIds?.add &&
						this.table.highlightIds.add(parseInt(recordId));
			});
		}
	};
	...
	getQueryOptions = (curLayer: LayersConfig) => {
		...
		if (searchText && curLayer.enableSearch && curLayer.searchFields) {
			const result = dataSourceUtils.getSQL(
				searchText,
				curLayer.searchFields,
				dataSource,
				curLayer?.searchExact
			);
			where = result.sql;
	...
	handleChange = (searchText: string) => {
		this.setState(
			{ searchText, isShowSuggestion: false, searchSuggestion: [] },
			() => {
				this.handleSubmit();
			}
		);
		...
		// } else {
		// 	this.setState(
		// 		{ searchText, isShowSuggestion: searchText?.length > 2 },
		// 		() => {
		// 			clearTimeout(this.suggestionsQueryTimeout);
	...
	renderSearchToolsHutch = (hint?: string) => {
				...
				{searchToolFlag ? (
					<div
						...
						<Button
							...
							onClick={(evt) => {
								this.setState({
									isOpenSearchPopper: !isOpenSearchPopper
								});
							}}>
							<SearchOutlined />
						</Button>
						<Popper
							...
							<div className="d-flex align-items-center table-popup-search m-2">
								...
								<TextInput
									...
									placeholder={
										hint || this.formatMessage("search")
									}
									onChange={(e) => {
	...
	getLoadingString = (time: number): string => {
		...
		if (time > 1 && time <= 2) {
			...
			loadingString = this.formatMessage("lastUpdateTime", {
				updateTime: time
			});
		}
	...
	resetAutoRefreshTimes = (interval: number, showLoading = false) => {
		...
		this.resetAutoRefreshTime = setTimeout(() => {
			if (showLoading && interval > 0) {
				_this.setState({
					autoRefreshLoadingString:
						_this.formatMessage("lastUpdateAFewTime")
				});
			}
			_this.setState({
	...
	renderLoading = (showLoading: boolean, interval: number) => {
		const { autoRefreshLoadingString, bottomResponsiveFlag, widgetWidth } =
			this.state;
		const countContainerWidth = this.refs.countContainer?.clientWidth || 0;
		const refreshMaxWidth = bottomResponsiveFlag
			? 20
			: widgetWidth - countContainerWidth - 16;
		return (
			<div
				className="position-absolute refresh-loading-con d-flex align-items-center"
				style={{ maxWidth: refreshMaxWidth }}>
				{showLoading && <div className="loading-con" />}
				{interval > 0 && (
					<div
						className="flex-grow-1 auto-refresh-loading text-truncate"
						title={
							!bottomResponsiveFlag && autoRefreshLoadingString
						}>
						{bottomResponsiveFlag ? (
							<Tooltip
								title={autoRefreshLoadingString}
								showArrow
								placement="top-end">
								<Button
									icon
									size="sm"
									type="tertiary"
									className="d-inline jimu-outline-inside border-0 p-0">
	...
	renderTableCount = (tableTotal: number, tableSelected: number) => {
		...
		const countMaxWidth = bottomResponsiveFlag
			? widgetWidth - 36
			: widgetWidth / 2;
		return (
			<div
				ref="countContainer"
				className="position-absolute table-count d-flex align-items-center"
				style={{ maxWidth: countMaxWidth }}>
				<div className="flex-grow-1 total-count-text text-truncate">
					{this.formatMessage("tableCount", {
						total: tableTotal,
	...
	getUsedFields = (
		...
		if (dataSource) {
			...
			if (popupInfo) {
				const popupAllFieldInfos = popupInfo.fieldInfos || [];
				const filteredPopupFieldInfos = popupAllFieldInfos.filter(
					(item) => schemaFieldsKeys.includes(item.fieldName)
				);
				...
				const layerDefinitionFields = (
					dataSource as FeatureLayerDataSource
				)?.getLayerDefinition()?.fields;
				const useFields =
					layerDefinitionFields?.length > 0
						? layerDefinitionFields
						: curLayer.allFields;
				const filteredUseFields = (useFields as any[]).filter((item) =>
					schemaFieldsKeys.includes(item.jimuName || item.name)
				);
		...
		const isHonorWebmap =
			curLayer.layerHonorMode === LayerHonorModeType.Webmap;
		const actionUsedFields = isHonorWebmap
			? popupAllFields.map((item) => item.value as string)
			: curLayer.tableFields.map((item) => item.jimuName);
		return { popupAllFields, actionUsedFields };
	};

	renderButtonToolList = (
		curLayer: LayersConfig,
		dataSource: QueriableDataSource
	) => {
		const {
		...
		const isHonorWebmap =
			curLayer.layerHonorMode === LayerHonorModeType.Webmap;
		const advancedTableField: ClauseValuePair[] = curLayer.tableFields?.map(
			(item) => {
				return { value: item.name, label: item.alias };
			}
		);
		const { popupAllFields, actionUsedFields } = this.getUsedFields(
			curLayer,
			dataSource
		);
		const dataSourceLabel = dataSource?.getLabel();
		const dataName = this.formatMessage("tableDataActionLabel", {
			layer: dataSourceLabel || ""
		});

		return (
			<div className="hutch-h4 top-button-list">
				{curLayer.enableSelect && (
					<div className="top-button ml-2">
						<Button
							...
							disabled={
								!tableLoaded || emptyTable || !hasSelected
							}>
							{selectQueryFlag ? (
								<MenuOutlined />
							) : (
								<ShowSelectionOutlined autoFlip />
							)}
						</Button>
					</div>
				)}
				...
				<div className="top-button ml-2">
					<AdvancedSelect
						...
						fluid
						staticValues={
							isHonorWebmap ? popupAllFields : advancedTableField
						}
						sortValuesByLabel={false}
						isMultiple
						selectedValues={
							tableShowColumns || initSelectTableFields
						}
					...
					<Fragment>
						{allLayersConfig.map((item) => {
							...
							const dataActionDropdown =
								isCurrentDataSource && !emptyTable ? (
	...
	renderDropdownToolList = (
		...
		return (
			<div className="hutch-h3 d-flex ml-2 dropdown-list-container">
						...
						<DropdownItem
							...
							<AdvancedSelect
								...
								buttonProps={{
									arrow: false,
									type: "tertiary",
									className: "p-0 table-hide-hover-color"
								}}
								fluid
								staticValues={
									isHonorWebmap
	...
	render() {
		...
		if (curLayer?.dataActionDataSource)
			dataSource = curLayer.dataActionDataSource as QueriableDataSource;
		...
		const dataSourceLabel = dataSource?.getLabel();
		const outputDsWidgetId =
			appConfigUtils.getWidgetIdByOutputDataSource(useDataSource);
		...
		const partProps = {
			...
			isHeightAuto,
			isWidthAuto,
			headerFontSetting: curLayer?.headerFontSetting,
			emptyTable,
			horizontalTag
		};
		const searchOn =
			curLayer?.enableSearch && curLayer?.searchFields?.length !== 0;
		const toolListNode = mobileFlag
		...
		return (
			<div
				className={classes}
				css={getStyle(theme, mobileFlag, searchOn, partProps)}
				ref={(el) => (this.refs.currentEl = el)}>
				<div className="surface-1 border-0 h-100">
					<div className="table-indent">
						{searchOn &&
							// this.renderSearchTools(curLayer?.searchHint)
							this.renderSearchToolsHutch(curLayer?.searchHint)}
						<div
							className={`hutch-h3 d-flex ${
								horizontalTag
									? "horizontal-tag-list"
									: "dropdown-tag-list"
							}`}>
