...
> {
	...
	static mapExtraStateProps = (
		...
		return {
			...
			activeTabId:
				state &&
				state.appStateInBuilder?.widgetsState[props.id]?.activeTabId
		};
	};
	...
	updateDsHash = (layersConfig: LayersConfig[]) => {
		...
		layersConfig.forEach((item) => {
			...
			const newUpdateDataSource = Immutable(useDataSource).set(
				"fields",
				usedFields
			);
	...
	dataSourceChangeSave = (useDataSources: UseDataSource[]) => {
		...
		const selectedDs = this.dsManager.getDataSource(
			currentIMUseDs.dataSourceId
		);
		const allFields = selectedDs && selectedDs.getSchema();
		const layerDefinition = (
			selectedDs as FeatureLayerDataSource
		)?.getLayerDefinition();
		...
		const allFieldsDetails = allFields?.fields
			? Object.values(allFields?.fields)
			: [];
		const fieldsConfig = layerDefinition?.fields || [];
		let initTableFields = allFieldsDetails
			.filter((item) => !defaultInvisible.includes(item.jimuName))
			.map((item) => {
				const orgField = fieldsConfig.find(
					(field) => field.name === item.jimuName
				);
		...
		if (
			...
			initTableFields = initTableFields.filter((item) =>
				popupVisibleFieldNames.includes(item.name)
			);
		}
		...
		this.dsManager
			...
			.then((currentDs) => {
				const layerItem: LayersConfig = {
					...
					useDataSource: curIMUseDsWithFields.asMutable({
						deep: true
					}),
				...
				if (currentLayer) {
					...
					const _conf = this.props.config.layersConfig.asMutable({
						deep: true
					});
	...
	optionChangeSave = (prop: string, value: any) => {
		...
		if (currentLayer) {
			...
			const newConfig = orgConfig.setIn(
				["layersConfig", this.index.toString()],
				{ ...currentLayer, [prop]: value }
			);
			const config = {
				id: this.props.id,
				config: newConfig
	...
	dataSourceFieldsChange = (
		updateDataSource: UseDataSource,
		updateInfo: { id: string; usedFields: string[] }
	) => {
		const usedConfigs = this.props.config.layersConfig.filter(
			(item) =>
				updateDataSource?.dataSourceId ===
				item?.useDataSource?.dataSourceId
		);
		let usedFields = [];
		...
		const index = useDataSources.findIndex(
			(item) => item.dataSourceId === updateDataSource?.dataSourceId
		);
		const newUseDataSources = useDataSources.asMutable({ deep: true });
		const newUpdateDataSource = Immutable(updateDataSource).set(
			"fields",
			newUsedFields
		);
		newUseDataSources[index] = newUpdateDataSource.asMutable({
			deep: true
		});
	...
	onCreateDataSourceCreatedOrFailed = (
		dataSourceId: string,
	...
	isDataSourceAccessible = (dataSourceId: string) => {
		const dataSourceIsInProps =
			this.props.useDataSources?.filter(
				(useDs) => dataSourceId === useDs.dataSourceId
			).length > 0;
		return (
			this.state.dataSources[dataSourceId] !== null && dataSourceIsInProps
		);
	};
	...
	setSidePopperAnchor = (index?: number, newAdded = false) => {
		...
		if (newAdded) {
			node =
				this.sidePopperTrigger.current.getElementsByClassName(
					"add-table-btn"
				)[0];
		} else {
			node = this.sidePopperTrigger.current.getElementsByClassName(
				...
				<div className="widget-setting-table h-100">
					{this.props.useDataSources?.map((useDs, index) => {
						return (
							<LayerConfigDataSource
								...
								useDataSource={useDs}
								onCreateDataSourceCreatedOrFailed={
									this.onCreateDataSourceCreatedOrFailed
								}
							/>
						);
					})}
					...
					{itemsLength > 0 && (
						<SettingSection
							...
							<SettingRow className="arrange_container">
								<Tooltip
									title={this.formatMessage("tabs")}
									placement="bottom">
									<Button
										onClick={() => {
											this.switchTableType(
												TableArrangeType.Tabs
											);
										}}
										icon
										size="sm"
					...
					<SidePopper
						...
						<LayerConfig
							...
							onDataSourceFieldsChange={
								this.dataSourceFieldsChange
							}
